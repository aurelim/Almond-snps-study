...
path = "../results/"
liste_Rep = sample(1:500, 500, replace = F)
...
omega_res = do.call(rbind, lapply(1:nrow(liste_idx), function(i) {
  pop = liste_idx$Var1[i]
  rep = liste_idx$Var2[i]

  file = paste0(path, pop, "/OmegaPlus_Report.", pop,".", rep, ".", window_size)
  tmp = readLines(file)
  tmp2 = tmp[!grepl("^//", tmp)]
...
sampled_data <- omega_res %>%
  group_by(population) %>%
  slice_sample(n = 5000) %>% 
  dplyr::rename(Omega = "V2")


p = sampled_data %>% 
  ggplot(aes(x = population, y = Omega, color = population))+
...
ggsave(
  paste0("../results/distri_omega.", window_size, ".png"),
  plot = p,
  width = 10,
  height = 20,
  units = "cm",
