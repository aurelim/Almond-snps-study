...
def write_vcf(vcf_file, positions, samples):
    ...
    with open(vcf_file, 'w') as file:
        ...
        # Écrire les positions et les génotypes dans le fichier VCF
        for chrom, pos_list in positions.items():
            for i, pos in enumerate(pos_list):
                # Le génotype de référence est déterminé par l'échantillon 1_1
                ref_allele_1 = reference_sample[i*2]  # Allèle 1 de 1_1
                ref_allele_2 = reference_sample[i*2+1]  # Allèle 2 de 1_1

                # On convertit les allèles en nucléotides avec notre encodage
                ref_allele_1 = nucleotide_encoding[ref_allele_1]
                ref_allele_2 = nucleotide_encoding[ref_allele_2]

                # Par convention, on garde un seul allèle pour le champ REF (premier allèle de 1_1)
                ref = ref_allele_1
                alt_set = set()  # Pour garder les allèles alternatifs uniques
                ...
                for sample_name, genotype in samples.items():
                    ...
                    # Conversion des génotypes en nucléotides
                    nuc1 = nucleotide_encoding[allele1]
                    nuc2 = nucleotide_encoding[allele2]

                    # Ajouter les allèles alternatifs différents de la référence
                    if nuc1 != ref:
                        alt_set.add(nuc1)
                    if nuc2 != ref:
                        alt_set.add(nuc2)

                    # Générer le format génotype VCF (utilisation du format 0|1, etc.)
                    geno = f"{0 if nuc1 == ref else 1}|{0 if nuc2 == ref else 1}"
                    genotypes.append(geno)

                # L'allèle alternatif (ALT) est la combinaison des allèles différents de REF
